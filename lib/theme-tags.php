...
function emanon_google_tag_manage() {
...
function emanon_google_tag_manage_noscript() {
  $tag_manage_id = get_theme_mod( 'tag_manage_id', '' );
  if ( $tag_manage_id ) {
...
function emanonn_entry_meta_category() {
  ...
  } else if ( is_emanon_display_category_list() && is_home() && is_front_page() || is_emanon_display_category_list() && is_search() ) {
...
/*----------------------------------------------------------------------------------*/
// Site name
function get_emanon_footer_copy() {
  $display_footer_copy = get_theme_mod( 'display_footer_copy', get_bloginfo( 'name' ) );
  return strip_tags( $display_footer_copy );
}

// Powered by emanon
function emanon_footer_copy() {
	$powered_by = '<br class="br-sp">Powered by <a href="https://wp-emanon.jp/" title="ワードプレス テーマ Emanon" target="_blank">WordPressテーマ Emanon</a></small>';
	$powered_by = apply_filters( 'emanon_footer_custom', $powered_by );
  echo '<div class="copyright">' . "\n";
  echo '<small>&copy;&nbsp;<a href="' . esc_url( home_url() ) . '">' . esc_html( get_emanon_footer_copy() ) . '</a>' . $powered_by . "\n";
  echo '</div>' . "\n";
}
